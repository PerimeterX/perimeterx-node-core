...
class PxClient {
    ...
    async fetchRemoteConfig(appId, remoteConfigToken) {
        const callData = {
            url: `https://sapi-${appId}.perimeterx.net/config/`,
            headers: { 'Authorization': `Bearer ${remoteConfigToken}`, 'Accept-Encoding': '' },
            timeout: 20000,
        };

        const response = await fetch(callData.url, { headers: callData.headers, timeout: callData.timeout });
        const remoteConfigObject = await response.json();
