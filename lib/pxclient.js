...
class PxClient {
    ...
    generateActivity(activityType, details, ctx, config) {
        ...
        this.addAdditionalFieldsToActivity(activityType, details, ctx);
    ...
    addAdditionalFieldsToActivity(activityType, details, ctx) {
        if (ctx.additionalFields && ctx.additionalFields.loginCredentials) {
            const { loginCredentials } = ctx.additionalFields;
            if (activityType !== ActivityType.ADDITIONAL_S2S) {
                details[CI_USERNAME_FIELD] = loginCredentials.username || undefined;
                details[CI_PASSWORD_FIELD] = loginCredentials.password || undefined;
            }
