...
class PxClient {
     ...
     */
    sendToPerimeterX(activityType, details, pxCtx, config) {
    ...
    sendEnforcerTelemetry(updateReason, config) {
        ...
        this.pxLogger.debug('Sending telemetry activity to perimeterx servers');
    ...
    submitActivities(config, cb) {
        if (this.activitiesBuffer.length > 0) {
            ...
            this.pxLogger.debug('Sending activities to perimeterx servers');
    ...
    callServer(data, path, headers, config, cb) {
        pxHttpc.callServer(data, headers, path, 'activities', config, this.pxLogger);
        if (cb) {
            cb();
        }
    }
