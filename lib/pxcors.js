...
function setCorsBlockHeaders(request, pxConfig, pxCtx) {
    ...
    const customCorsHeaders = pxConfig.CORS_CREATE_CUSTOM_BLOCK_RESPONSE_HEADERS;

    if (customCorsHeaders) {
        try {
            corsHeaders = customCorsHeaders(pxCtx, request);
        } catch (e) {
            pxConfig.logger.debug(`Caught error in px_custom_cors_block_response_headers: ${e}`);
        }
    }
