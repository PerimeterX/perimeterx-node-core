...
class PxContext {
    constructor(config, req, additionalFields) {
        ...
        if (pxUtil.isGraphql(req, config)) {
            ...
            this.graphqlData = Array.isArray(req.body) ?
                req.body.map(pxUtil.getGraphqlData) :
                [pxUtil.getGraphqlData(req.body)];
    ...
    isSensitiveGraphqlOperation(config) {
        return this.graphqlData &&
            this.graphqlData.some(graphqlOperation =>
                graphqlOperation &&
                (config.SENSITIVE_GRAPHQL_OPERATION_TYPES.includes(graphqlOperation.operationType) ||
                config.SENSITIVE_GRAPHQL_OPERATION_NAMES.includes(graphqlOperation.operationName))
            );
    }
