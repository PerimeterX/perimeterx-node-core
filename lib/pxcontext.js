...
class PxContext {
    constructor(config, req, additionalFields) {
        ...
        if (mobileHeader !== undefined) {
            ...
            if (config.CUSTOM_COOKIE_HEADER && req.headers[config.CUSTOM_COOKIE_HEADER]) {
                ...
                if (customCookies) {
                    const cookiesString = [JSON.stringify(cookies), JSON.stringify(customCookies)].filter(Boolean).join(constants.COOKIE_SEPARATOR);
    ...
    mergeCookieStrings(cookieString) {
        const [before, after] = cookieString.split(constants.COOKIE_SEPARATOR).map(JSON.parse);
