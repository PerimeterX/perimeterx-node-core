...
const { RISK_API_URI } = require('./utils/constants');
 ...
 */
function callServer(ctx, pxConfig, callback) {
    const data = buildRequestData(ctx, pxConfig);
    ...
    return pxHttpc.callServer(data, reqHeaders, RISK_API_URI, 'query', pxConfig, callback);
}

function buildRequestData(ctx, pxConfig) {
    const { px_sensitive_headers, px_module_mode } = pxConfig.Config;
    ...
    const uuid = ctx.uuid || '';
    const headers = pxUtil.formatHeaders(ctx.headers, px_sensitive_headers);
    const httpVersion = ctx.httpVersion;
    const riskMode =
        (px_module_mode === ModuleMode.MONITOR && !ctx.shouldBypassMonitor) || ctx.monitoredRoute
            ? 'monitor'
            : 'active_blocking';
    ...
    pxUtil.prepareCustomParams(pxConfig, data.additional, ctx.originalRequest);
 ...
 * @param {Function} callback - callback function.
 */
function evalByServerCall(ctx, pxConfig, callback) {
    if (!ctx.ip || !ctx.headers) {
        pxConfig.Logger.error('perimeterx score evaluation failed. bad parameters.');
        return callback(ScoreEvaluateAction.UNEXPECTED_RESULT);
    }
    pxConfig.Logger.debug(`Evaluating Risk API request, call reason: ${ctx.s2sCallReason}`);
    callServer(ctx, pxConfig, (err, res) => {
        if (err) {
            if (err === 'timeout') {
                ctx.passReason = PassReason.S2S_TIMEOUT;
                return callback(ScoreEvaluateAction.S2S_TIMEOUT_PASS);
            }
            pxConfig.Logger.error(
        ...
        const action = isBadRiskScore(res, ctx, pxConfig);
        /* score response invalid - pass traffic */
        if (action === -1) {
            pxConfig.Logger.error('perimeterx server query response is invalid');
            return callback(ScoreEvaluateAction.UNEXPECTED_RESULT);
        }
 ...
 *
 */
function isBadRiskScore(riskResponse, ctx, pxConfig) {
    ...
    if (score >= pxConfig.Config.px_blocking_score) {
