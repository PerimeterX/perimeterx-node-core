...
const { CI_USERNAME_FIELD, CI_PASSWORD_FIELD, CI_VERSION_FIELD, CI_SSO_RISK_STEP_FIELD } = require('./utils/constants');
...
function buildRequestData(ctx, config) {
    ...
    if (ctx.additionalFields && ctx.additionalFields.loginCredentials) {
        ...
        if (loginCredentials.version === CIVersion.MULTISTEP_SSO) {
            data.additional[CI_SSO_RISK_STEP_FIELD] = loginCredentials.ssoStep;
        }
    }
