...
const { CI_USERNAME_FIELD, CI_PASSWORD_FIELD, CI_VERSION_FIELD, CI_SSO_STEP_FIELD, GQL_OPERATION_TYPE_FIELD, GQL_OPERATION_NAME_FIELD,
...
function buildRequestData(ctx, config) {
    ...
    if (ctx.graphqlData) {
        ...
        data.additional[GQL_OPERATION_TYPE_FIELD] = ctx.graphqlData[0] && ctx.graphqlData[0].operationType;
        data.additional[GQL_OPERATION_NAME_FIELD] = ctx.graphqlData[0] && ctx.graphqlData[0].operationName;
    }
