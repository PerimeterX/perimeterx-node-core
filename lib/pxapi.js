 ...
 */
function callServer(pxCtx, config, pxLogger, callback) {
    const ip = pxCtx.ip;
    ...
    const uri = pxCtx.uri || '/';
    const headers = pxUtil.formatHeaders(pxCtx.headers, config.SENSITIVE_HEADERS);
    ...
    return pxHttpc.callServer(data, reqHeaders, config.SERVER_TO_SERVER_API_URI, 'query', config, pxLogger, callback);
}
 ...
 * @param {Function} callback - callback function.
 */
function evalByServerCall(pxCtx, config, pxLogger, callback) {
    ...
    callServer(pxCtx, config, pxLogger, (err, res) => {
        if (err) {
            if (err === 'timeout') {
            ...
            pxCtx.passReason = config.PASS_REASON.REQUEST_FAILED;
            return callback(config.SCORE_EVALUATE_ACTION.UNEXPECTED_RESULT);
        }
        pxCtx.pxhd = res.pxhd;
        const action = isBadRiskScore(res, pxCtx, config);
 ...
 *
 */
function isBadRiskScore(res, pxCtx, config) {
    if (!res || !pxUtil.verifyDefined(res.score) || !res.action) {
        pxCtx.passReason = config.PASS_REASON.INVALID_RESPONSE;
        return -1;
    }
    ...
    if (score >= config.BLOCKING_SCORE) {
        ...
        pxCtx.passReason =  config.PASS_REASON.S2S;
        return 1;
    }
}
