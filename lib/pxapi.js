...
function callServer(pxCtx, callback) {
    ...
    if (config.ENRICH_CUSTOM_PARAMETERS) {
        ...
        for (let param in riskCustomParams) {
            if (param.match(/^custom_param\d$/) && riskCustomParams[param] != '') {
                data.additional[param] = riskCustomParams[param];
            }
        }
    }
    ...
    if(pxCtx.getHmac()){
        data.additional["px_cookie_hmac"] = pxCtx.getHmac();
    }

    if(pxCtx.decodedCookie){
        data.additional.pxCookie = pxCtx.decodedCookie;
    }
