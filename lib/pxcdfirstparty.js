...
class PxCdFirstParty {
    ...
    handleFirstPartyRequest(req, res, cb) {
        ...
        const ipAddress = extractIP(this._config, req);
        ...
        //CD only sensor endpoint
        if (requestUrl.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_CD_SENSOR_PATH}`)) {
            return pxProxy.getCDClient(req, this._config, ipAddress, cb);
        }
        if (requestUrl.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_CD_XHR_PATH}`)) {
            return pxProxy.sendCDXHR(req, this._config, ipAddress, cb);
        }
        return cb();
    }
}
