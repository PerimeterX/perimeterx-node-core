        ...
        if (pxCookie && pxCtx.cookieOrigin === 'header') {
            if (pxCookie.match(/^\d+$/)) {
                ...
                if (pxCtx.originalToken) {
                    originalTokenValidator.evalCookie(pxCtx, config, pxLogger);
                }
                return config.SCORE_EVALUATE_ACTION.SPECIAL_TOKEN;
            }
        }

        const cookie = pxCookieFactory(pxCtx, config, pxLogger);
 ...
 */
function pxCookieFactory(pxCtx, config, pxLogger) {
    if (pxCtx.cookieOrigin == 'cookie') {
        return (pxCtx.cookies['_px3'] ? new CookieV3(pxCtx, config, pxLogger) : new CookieV1(pxCtx, config, pxLogger));
    } else {
        return (pxCtx.cookies['_px3'] ? new TokenV3(pxCtx, config, pxCtx.cookies['_px3'], pxLogger) : new TokenV1(pxCtx, config, pxCtx.cookies['_px'], pxLogger));
    }
}
