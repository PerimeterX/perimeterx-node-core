...
class Payload {
    ...
    decrypt() {
        try {
            ...
            if (data.length !== 3) {
                this.pxLogger.debug('invalid cookie format - wrong number of parts');
                return '';
            }
            ...
            if (!iterations) {
                this.pxLogger.debug('invalid cookie format - iterations value is not a number');
                return '';
            }

            /* iterations value is not in the legit range */
            if (iterations > 5000 || iterations < 500) {
                this.pxLogger.debug('invalid cookie format - iterations out of bounds');
                return '';
            }

            /* salt value is not as expected */
            if (!data[0] || typeof data[0] !== 'string' || data[0].length > 100) {
                this.pxLogger.debug('invalid cookie format - invalid salt value');
                return '';
            }

            /* cookie value is not as expected */
            if (!encryptedCookie || typeof encryptedCookie !== 'string') {
                this.pxLogger.debug('invalid cookie format - no cookie value');
                return '';
            }
            ...
            this.pxLogger.error('Error while decrypting Perimeterx cookie: ' + err.message);
            return '';
        }
    }
    ...
    isHmacValid(hmacStr, cookieHmac) {
        try {
            ...
            this.pxLogger.error('Error while validating Perimeterx cookie: ' + err.stack);
        }
    }
}
