...
function getClient(req, pxConfig, ip, cb) {
     ...
     if (!pxConfig.FIRST_PARTY_ENABLED) {
        ...
        request.get(callData, (error, response, body) => {
            if (error || !response) {
                pxLogger.debug(`Error getting client file: ${error || response}`);
                return cb(e);
            }
            res = {
                status: 200,
    ...
    parseBody(req).then(() => {
        ...
        if (req.method === "POST") {
            request.post(callData, (error, response, body) => {
                if (error || !response) {
                    pxLogger.debug(`Error sending XHR request using POST: ${error || response}`);
                    return cb(e);
                }
                res = {
                    status: 200,
            ...
            request.get(callData, (error, response, body) => {
                if (error || !response) {
                    pxLogger.debug(`Error sending XHR request using GET: ${error || response}`);
                    return cb(e);
                }
