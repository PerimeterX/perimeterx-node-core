 ...
 *
 */
function getCaptcha(req, config, ip, reversePrefix, pxLogger, cb) {
    let res = {};
    if (!config.FIRST_PARTY_ENABLED) {
        ...
        pxLogger.debug(`Forwarding request from ${req.originalUrl} to xhr at ${config.CAPTCHA_HOST}${pxRequestUri}`);
        const callData = {
 ...
 *
 */
function getClient(req, config, ip, pxLogger, cb) {
    let res = {};
    if (!config.FIRST_PARTY_ENABLED) {
        res = {
            status: 200,
        ...
        pxLogger.debug(`Forwarding request from ${req.originalUrl.toLowerCase()} to client at ${config.CLIENT_HOST}${clientRequestUri}`);
        const callData = {
 ...
 *
 */
function sendXHR(req, config, ip, reversePrefix, pxLogger, cb) {
    let res = {};
    let vid;
    ...
    // handle proxy
    parseBody(req, pxLogger).then(() => {
        const searchMask = `/${reversePrefix}${config.FIRST_PARTY_XHR_PATH}`;
        const regEx = new RegExp(searchMask, 'ig');
        const pxRequestUri = req.originalUrl.replace(regEx, '');
        pxLogger.debug(`Forwarding request from ${req.originalUrl} to xhr at ${config.COLLECTOR_HOST}${pxRequestUri}`);
