...
function VerifyRegexAndString(match, matchWith) {
    let isMatch = false;
    if (match instanceof RegExp && matchWith.match(match)) {
        isMatch = true;
    }

    if (typeof match === 'string' && matchWith.toLowerCase() === match.toLowerCase()) {
        isMatch = true;
    }
