...
function parseGraphqlBody(query) {
    ...
    while ((match = pattern.exec(query)) !== null) {
        ...
        // if two operations has the same name, the query is illegal.
...
function isSensitiveGraphqlOperation(graphqlData, config) {
    return graphqlData &&
        config.SENSITIVE_GRAPHQL_OPERATION_TYPES.includes(graphqlData.operationType) &&
        config.SENSITIVE_GRAPHQL_OPERATION_NAMES.includes(graphqlData.operationName);
}
...
function getGraphqlData(graphqlBodyObject) {
    ...
    return new GraphqlData(parsedData[selectedOperationName], selectedOperationName, graphqlBodyObject.variables);
}
