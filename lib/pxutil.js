...
function prepareCustomParams(config, dict, originalRequest) {
    ...
    if (config.px_enrich_custom_parameters) {
        const enrichedCustomParams = config.px_enrich_custom_parameters(customParams, originalRequest);
...
function extractIP(req, ipHeaders, getUserIpFunc, logger) {
    let ip;
    if (Array.isArray(ipHeaders)) {
        ipHeaders.some((ipHeader) => {
            try {
                ...
                logger.debug('Failed to use px_ip_headers from config.');
            }
        });
    } else {
        ip = typeof getUserIpFunc === 'function' && getUserIpFunc(req);
    }
    if (ip && net.isIP(ip) > 0) {
        return ip;
    }
...
function isReqInMonitorMode(pxConfig, pxCtx) {
    return (
        (pxConfig.Config.px_module_mode === ModuleMode.MONITOR && !pxCtx.shouldBypassMonitor) || pxCtx.monitoredRoute
    );
}
