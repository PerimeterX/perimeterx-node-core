 ...
 * @param {Array} cookieHeader - The received http request cookie header after parsing by delimiter.
 */
function extractCookieNames(cookies) {
 ...
 *
 * */
function extractCookieValue(cookieName, cookieArray) {
    let result;
...
function prepareCustomParams(config, dict) {
    ...
    if (config.ENRICH_CUSTOM_PARAMETERS) {
        const riskCustomParams = config.ENRICH_CUSTOM_PARAMETERS(customParams);
        for (const param in riskCustomParams) {
            if (param.match(/^custom_param\d$/) && riskCustomParams[param] != '') {
                dict[param] = riskCustomParams[param];
            }
        }
    }
}
...
module.exports = {
    ...
    extractCookieNames,
    prepareCustomParams,
    extractCookieValue: extractCookieValue
};
