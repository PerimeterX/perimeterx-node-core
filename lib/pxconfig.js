...
PX_DEFAULT.CONFIG_PATH = '';

class PxConfig {
    static init(params, pxClient) {
        config = PxConfig.mergeConfigFileParams(params);
    ...
    static mergeConfigFileParams(configParams) {
        try {
            const filepath = configParams['configFilePath'];
            const fileConfig = PxConfig.readConfigFile(filepath);
            configParams = utils.mergeMaps(configParams, fileConfig);
        } catch(err) {
            console.error('Failed while trying to read the config json file, ' + err);
        }
        return PxConfig.mergeDefaults(configParams);
    }
    ...
    static readConfigFile(filepath) {
        ...
        if (!data) {
            ...
            console.error('Failed while trying to read the config json file.');
        }
        try{
    ...
    static mergeDefaults(params) {
        ...
        PX_DEFAULT.CONFIG_PATH = this.configurationsOverriding(PX_DEFAULT, params, 'CONFIG_PATH', 'configFilePath');
