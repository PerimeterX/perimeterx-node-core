...
class PxConfig {
    ...
    mergeParams(params) {
        ...
        const configKeyMapping = [['ENABLE_MODULE', 'enableModule'], ['PX_APP_ID', 'pxAppId'], ['COOKIE_SECRET_KEY', 'cookieSecretKey'], ['AUTH_TOKEN', 'authToken'], ['PROXY_URL', 'proxy'],
            ...
            ['TELEMETRY_COMMAND_HEADER', 'telemetryCommandHeader'], ['CUSTOM_TEMPLATE_ROOT', 'customTemplateRoot'], ['CUSTOM_TEMPLATE_DATA', 'customTemplateData'], ['FILTER_BY_IP', 'filterByIP'], ['FILTER_BY_USERAGENT', 'filterByUserAgent'], ['FILTER_BY_METHOD', 'filterByMethod'],
            ['EXTERNAL_ACTIVITIES', 'externalActivities'], ['PXHD_SECURE', 'pxhdSecure'], ['BACKEND_URL', 'backendUrl']];
        ...
        if(this.PX_DEFAULT['BACKEND_URL']) {
            this.PX_INTERNAL.SERVER_HOST_SCHEME = this.PX_DEFAULT['BACKEND_URL'].startsWith('http://') ? 'http' : 'https';
            this.PX_INTERNAL.SERVER_HOST = this.PX_DEFAULT['BACKEND_URL'].replace(/^(https:\/\/)|^(http:\/\/)/,"");
        }
        else {
            this.PX_INTERNAL.SERVER_HOST_SCHEME = 'https'
        ...
        // set collector host and module version
