...
class PxConfig {
    constructor(params, logger) {
        ...
        else if (this.config.HTTPS_AGENT_KEEP_ALIVE_TIMEOUT_MS !== 0) {
            this.config.agent = new https.Agent({ keepAlive: true, timeout: this.config.HTTPS_KEEP_ALIVE_AGENT_TIMEOUT_MS });
        }
    ...
    mergeParams(params) {
        ...
        const configKeyMapping = [
            ...
            ['ACTIVITIES_TIMEOUT_MS', 'activitiesTimeout'],
            ['CUSTOM_REQUEST_HANDLER', 'customRequestHandler'],
            ['GET_USER_IP', 'getUserIp'],
            ['BLOCKING_SCORE', 'blockingScore'],
            ['IP_HEADERS', 'ipHeaders'],
            ['SEND_PAGE_ACTIVITIES', 'sendPageActivities'],
            ['SENSITIVE_HEADERS', 'sensitiveHeaders'],
            ['DEBUG_MODE', 'debugMode'],
            ['MAX_BUFFER_LEN', 'maxBufferLength'],
            ['JS_REF', 'jsRef'],
            ['CSS_REF', 'cssRef'],
            ['CUSTOM_LOGO', 'customLogo'],
            ['SENSITIVE_ROUTES', 'sensitiveRoutes'],
            ['WHITELIST_ROUTES', 'whitelistRoutes'],
            ['ENFORCED_ROUTES', 'enforcedRoutes'],
            ['MONITORED_ROUTES', 'monitoredRoutes'],
            ['DYNAMIC_CONFIGURATIONS', 'dynamicConfigurations'],
            ['MODULE_MODE', 'moduleMode'],
            ['FIRST_PARTY_ENABLED', 'firstPartyEnabled'],
            ['ADDITIONAL_ACTIVITY_HANDLER', 'additionalActivityHandler'],
            ['ENRICH_CUSTOM_PARAMETERS', 'enrichCustomParameters'],
            ['TESTING_MODE', 'testingMode'],
            ['WHITELIST_EXT', 'whitelistExt'],
            ['BYPASS_MONITOR_HEADER', 'bypassMonitorHeader'],
            ['ADVANCED_BLOCKING_RESPONSE', 'advancedBlockingResponse'],
            ['TELEMETRY_COMMAND_HEADER', 'telemetryCommandHeader'],
            ['CUSTOM_TEMPLATE_ROOT', 'customTemplateRoot'],
            ['CUSTOM_TEMPLATE_DATA', 'customTemplateData'],
            ['FILTER_BY_IP', 'filterByIP'],
            ['FILTER_BY_USERAGENT', 'filterByUserAgent'],
            ['FILTER_BY_METHOD', 'filterByMethod'],
            ['EXTERNAL_ACTIVITIES', 'externalActivities'],
            ['PXHD_SECURE', 'pxhdSecure'],
            ['BACKEND_URL', 'backendUrl'],
            ['CUSTOM_COOKIE_HEADER', 'customCookieHeader'],
            ['ENABLE_LOGIN_CREDS_EXTRACTION', 'enableLoginCredsExtraction'],
            ['LOGIN_CREDS_EXTRACTION', 'loginCredsExtraction'],
            ['ENABLE_RISK_RTT_LOGS', 'enableRiskRttLogs'],
            ['SEND_RISK_RTT_LOGS_ABOVE_MS', 'sendRiskRttLogsAboveMs'],
            ['HTTPS_AGENT_KEEP_ALIVE_TIMEOUT_MS', 'httpsAgentKeepAliveTimeoutMs']
        ];
...
function pxDefaultConfig(PX_INTERNAL) {
    return {
        ...
        ENABLE_RISK_RTT_LOGS: false,
        SEND_RISK_RTT_LOGS_ABOVE_MS: 1500,
        HTTPS_AGENT_KEEP_ALIVE_TIMEOUT_MS: 0
    };
}
...
const configSchemaMapper = {
    ...
    px_pxhd_secure: 'pxhdSecure',
    px_backend_url: 'backendUrl',
    px_enable_login_creds_extraction: 'enableLoginCredsExtraction',
    px_login_creds_extraction: 'loginCredsExtraction',
};
