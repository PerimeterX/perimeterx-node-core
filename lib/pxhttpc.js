 ...
 */
function callServer(data, headers, uri, callType, config, callback, ctx) {
    callback =
        callback ||
        ((err) => {
            err && config.logger.debug(`callServer default callback. Error: ${err}`);
        });
    const callData = {
    ...
    if (ctx && ctx.riskRttLogs) {
        ctx.riskRttLogs += `callServer:${Date.now() - ctx.startRiskRtt};`;
    }
    ...
    try {
        request.post(callData, config, function (err, response) {
            if (err) {
                if (err.toString().toLowerCase().includes('timeout')) {
                    if (ctx && ctx.riskRttLogs) {
                        ctx.riskRttLogs += `timeoutReceived:${Date.now() - ctx.startRiskRtt};`;
                    }
                    return callback('timeout');
                } else {
                    if (ctx && ctx.riskRttLogs) {
                        ctx.riskRttLogs += `errorReceived:${Date.now() - ctx.startRiskRtt};`;
                    }
                    return handleError(callback, S2SErrorReason.UNKNOWN_ERROR, `encountered error with risk api call: ${err}`, response);
                }
            }
            if (ctx && ctx.riskRttLogs) {
                ctx.riskRttLogs += `responseReceived:${Date.now() - ctx.startRiskRtt};`;
            }

            if (!response) {
                if (ctx && ctx.riskRttLogs) {
                    ctx.riskRttLogs += `handleErrorResponse:${Date.now() - ctx.startRiskRtt};`;
                }
                return handleError(
                    callback,
            ...
            if (response.statusCode === 200) {
                if (ctx && ctx.riskRttLogs) {
                    ctx.riskRttLogs += `handleOkResponse:${Date.now() - ctx.startRiskRtt};`;
                }
                return handleOkResponse(callback, response);
            }

            if (ctx && ctx.riskRttLogs) {
                ctx.riskRttLogs += `handleUnexpectedHttpResponse:${Date.now() - ctx.startRiskRtt};`;
            }
        ...
        if (ctx && ctx.riskRttLogs) {
            ctx.riskRttLogs += `unableToSendRequest:${Date.now() - ctx.startRiskRtt};`;
        }
