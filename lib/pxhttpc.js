 ...
 */
function callServer(data, headers, uri, callType, pxConfig, callback) {
    const { px_s2s_timeout, px_backend_url, px_batch_activities_timeout_ms } = pxConfig.Config;
    callback =
        callback ||
        ((err) => {
            err && pxConfig.Logger.debug(`callServer default callback. Error: ${err}`);
        });
    const callData = {
        url: `${px_backend_url}${uri}`,
        data: JSON.stringify(data),
        headers: headers,
    };

    callData.timeout = callType === 'query' ? px_s2s_timeout : px_batch_activities_timeout_ms;

    try {
        request.post(callData, pxConfig.ProxyAgent, function (err, response) {
            if (err) {
