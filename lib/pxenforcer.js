...
class PxEnforcer {
    ...
    pxPass(ctx, req) {
        ...
        if (this._config.ENABLE_ADDITIONAL_ACTIVITY_HEADER) {
            req.headers[Constants.DEFAULT_ADDITIONAL_ACTIVITY_HEADER_NAME] = JSON.stringify(
                this.pxClient.generateActivity(ActivityType.ADDITIONAL_S2S, { client_uuid: ctx.uuid }, ctx, this._config)
            );
        ...
        if (this._config.EXTERNAL_ACTIVITIES && req) {
            req.headers[Constants.PAGE_REQUESTED_HEADER_NAME] = JSON.stringify(
                this.pxClient.generateActivity(ActivityType.PAGE_REQUESTED, details, ctx, this._config),
            );
        } else {
            this.logger.debug('Sending page requested activity, pass_reason: ' + ctx.passReason);
