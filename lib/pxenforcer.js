...
class PxEnforcer {
    ...
    pxPass(ctx, req) {
        ...
        if (this._config.ENABLE_ADDITIONAL_ACTIVITY_HEADER) {
            const activity = this.pxClient.generateActivity('additional_s2s', {}, ctx, this._config);
            activity.headers = undefined;
            activity.details = { client_uuid: ctx.uuid, http_status_code: null, login_successful: null };
            req.headers[Constants.DEFAULT_ADDITIONAL_ACTIVITY_HEADER_NAME] = JSON.stringify(activity);
