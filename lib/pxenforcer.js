...
class PxEnforcer {
    ...
    verifyUserScore(ctx, callback) {
        ...
        try {
            ...
            ctx.riskRttLogs += `ip,uri:${Date.now() - startRiskRtt};`;
            const action = pxCookie.evalCookie(ctx, this._config);
            ctx.riskRttLogs += `evalCookie:${Date.now() - startRiskRtt};`;

            /* score did not cross threshold - pass traffic */
            if (action === ScoreEvaluateAction.GOOD_SCORE) {
                return callback(ScoreEvaluateAction.COOKIE_PASS_TRAFFIC);
            }
            ctx.riskRttLogs += `goodScoreAction:${Date.now() - startRiskRtt};`;
            ...
            ctx.riskRttLogs += `badScoreAction:${Date.now() - startRiskRtt};`;
