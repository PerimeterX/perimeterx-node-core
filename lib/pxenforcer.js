...
class PxEnforcer {
    ...
    enforce(req, res, cb) {
        ...
        if (userAgent && this._config.FILTER_BY_USERAGENT && this._config.FILTER_BY_USERAGENT.length > 0) {
            for (const ua of this._config.FILTER_BY_USERAGENT) {
                if (ua instanceof RegExp && userAgent.match(ua)) {
                    this.logger.debug(`Skipping verification for filtered user agent ${userAgent}`);
                    return cb();
                }

                if (typeof ua === 'string' && userAgent.toLowerCase() === ua.toLowerCase()) {
                    this.logger.debug(`Skipping verification for filtered user agent ${userAgent}`);
                    return cb();
                }
            }
    ...
    handleVerification(ctx, req, res, cb) {
        ...
        const shouldBypassMonitor =
            this._config.BYPASS_MONITOR_HEADER && req.headers[this._config.BYPASS_MONITOR_HEADER] === '1';
        if (
            verified ||
            ctx.monitoredRoute ||
            (this._config.MODULE_MODE === this._config.MONITOR_MODE.MONITOR && !shouldBypassMonitor)
        ) {
            return cb();
        }
