...
class PxEnforcer {
    ...
    enforce(req, res, cb) {
        ...
        const normalizedUrl = this._normalizeUrl(pxUtil.getFullUrlFromRequest(req));
        const NormalizedWholeUrl = normalizedUrl.href;
        req[requestUrlKeys.requestNormalizedUrl] = NormalizedWholeUrl;
        req[requestUrlKeys.isOriginalDifferentFromOriginal] = req.originalUrl !== NormalizedWholeUrl;
        ...
        if (NormalizedPath.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_VENDOR_PATH}`)) {
            // reverse proxy client
            return pxProxy.getClient(req, this._config, ipAddress, cb);
        }

        if (NormalizedPath.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_XHR_PATH}`)) {
            //reverse proxy xhr
            return pxProxy.sendXHR(req, this._config, ipAddress, this.reversePrefix, cb);
        }

        if (NormalizedPath.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_CAPTCHA_PATH}`)) {
