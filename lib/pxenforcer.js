...
class PxEnforcer {
    ...
    handleVerification(pxCtx, pxConfig, req, res, cb) {
        ...
        if (pxCtx.score < pxConfig.BLOCKING_SCORE) {
            ...
            let isJsonResponse = acceptHeaderValue && acceptHeaderValue.split(',').find((value) => value.toLowerCase() === "application/json") && pxCtx.cookieOrigin === "cookie";

            pxLogger.debug(`Enforcing action: ${pxCtx.fullBlockAction} page is served ${isJsonResponse ? "using advanced protection mode" : ""}`);
     ...
     */
    generateResponse(pxContext, pxConfig, jsonResponse = false, cb) {
        let template;
