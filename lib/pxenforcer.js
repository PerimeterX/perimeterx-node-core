...
class PxEnforcer {
    constructor(params, client) {
        ...
        this.fieldExtractorManager = this._createFieldExtractorManager(this.logger, this._config);
    }

    _createFieldExtractorManager(logger, config) {
        if (config.ENABLE_LOGIN_CREDS_EXTRACTION && config.LOGIN_CREDS_EXTRACTION.length > 0) {
            return new FieldExtractorManager(logger, config.LOGIN_CREDS_EXTRACTION);
        }
        return null;
    }
    ...
    verifyUserScore(ctx, callback) {
        ctx.startRiskRtt = Date.now();
        ctx.riskRtt = 0;
        ctx.riskRttLogs += `start;`;
        ...
        try {
            ...
            ctx.riskRttLogs += `ip,uri:${Date.now() - ctx.startRiskRtt};`;
            const action = pxCookie.evalCookie(ctx, this._config);
            ctx.riskRttLogs += `evalCookie:${Date.now() - ctx.startRiskRtt};`;

            /* score did not cross threshold - pass traffic */
            if (action === ScoreEvaluateAction.GOOD_SCORE) {
                return callback(ScoreEvaluateAction.COOKIE_PASS_TRAFFIC);
            }
            ctx.riskRttLogs += `goodScoreAction:${Date.now() - ctx.startRiskRtt};`;
            ...
            ctx.riskRttLogs += `badScoreAction:${Date.now() - ctx.startRiskRtt};`;
            ...
            pxApi.evalByServerCall(ctx, this._config, (action) => {
                ctx.riskRtt = Date.now() - ctx.startRiskRtt;
                ctx.riskRttLogs += `riskRttDetermined:${ctx.riskRtt};`;
            ...
            ctx.riskRtt = Date.now() - ctx.startRiskRtt;
            ctx.riskRttLogs += `errorOccured:${ctx.riskRtt};`;
    ...
    pxPass(ctx, req) {
        ...
        if (this._config.ENABLE_RISK_RTT_LOGS && ctx.riskRtt > this._config.SEND_RISK_RTT_LOGS_ABOVE_MS) {
            details['risk_rtt_logs'] = ctx.riskRttLogs;
        }
