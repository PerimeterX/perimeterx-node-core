...
const { requestUrlKeys } = require('./pxutil');
...
class PxEnforcer {
    ...
    enforce(req, res, cb) {
        ...
        const fullUrl = pxUtil.getFullUrlFromRequest(req);
        const requestNormalizedUrl = this._normalizeUrl(fullUrl).href;

        req[requestUrlKeys.requestNormalizedUrl] = requestNormalizedUrl;
        req[requestUrlKeys.isOriginalDifferentFromOriginal] = req.originalUrl !== requestNormalizedUrl;
        ...
        if (requestNormalizedUrl.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_VENDOR_PATH}`)) {
            // reverse proxy client
            return pxProxy.getClient(req, this._config, ipAddress, cb);
        }

        if (requestNormalizedUrl.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_XHR_PATH}`)) {
            //reverse proxy xhr
            return pxProxy.sendXHR(req, this._config, ipAddress, this.reversePrefix, cb);
        }

        if (requestNormalizedUrl.startsWith(`/${this.reversePrefix}${this._config.FIRST_PARTY_CAPTCHA_PATH}`)) {
    ...
    _normalizeUrl(originalUrl) {
        ...
        if (this._config.URL_DECODE_RESERVED_CHARACTERS) {
            try {
                decodedUrl = new URL(`${url.origin}${decodeURIComponent(url.pathname)}${url.search}`);
            } catch (e) {
    ...
    getActivityDetails(ctx) {
        ...
        if (ctx.isOriginalDifferentFromOriginal) {
