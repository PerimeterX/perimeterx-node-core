...
class PxEnforcer {
    constructor(params, client) {
        ...
        this.fieldExtractor = this._createFieldExtractor(this.logger, this._config);
    }

    _createFieldExtractor(logger, config) {
    ...
    _getAdditionalFields(req) {
        ...
        if (this.fieldExtractor) {
            additionalFields = this.fieldExtractor.ExtractFields(req);
        }
