...
class PxEnforcer {
    ...
    generateTemplate(pxContext, pxConfig, cb){
        ...
        pxHttpc.getCaptchaScript(template, (err, scriptBody) => {
            let htmlTemplate = '';

            // Mustache preparations
            mu.root = templatesPath;
            const props = this.getProps(pxContext, pxConfig, scriptBody);
            const compile = mu.compileAndRender('block_template.mustache', props);
    ...
    getProps(pxContext, pxConfig, scriptBody){
        let jsClientSrc = `//client.perimeterx.net/${pxConfig.PX_APP_ID}/main.min.js`;
        if (pxConfig.FIRST_PARTY_ENABLED) {
            jsClientSrc = `/${pxConfig.PX_APP_ID.substring(2).toLowerCase()}${pxConfig.FIRST_PARTY_VENDOR_PATH}`;
        };
