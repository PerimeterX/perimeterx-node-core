...
class PxEnforcer {
    ...
    pxPass(ctx, req) {
        ...
        if (ctx.passReason === PassReason.S2S_ERROR && ctx.s2sErrorInfo) {
            details['s2s_error_reason'] = ctx.s2sErrorInfo.errorReason;
            details['s2s_error_message'] = ctx.s2sErrorInfo.errorMessage;
            details['s2s_error_http_status'] = ctx.s2sErrorInfo.httpStatus;
            details['s2s_error_http_message'] = ctx.s2sErrorInfo.httpMessage;
        }
